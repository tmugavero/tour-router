...
function MarketCard({market,index}){const[open,setOpen]=useState(false);const p=market.profitability;return(
  <div onClick={()=>setOpen(!open)} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:8,padding:"12px 14px",cursor:"pointer",transition:"all 0.2s",borderColor:open?"rgba(245,158,11,0.3)":undefined}}>
    <div style={{display:"flex",alignItems:"center",gap:10}}>
      <div style={{width:28,height:28,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",background:market.has_artist_history?"rgba(245,158,11,0.15)":"rgba(6,182,212,0.15)",color:market.has_artist_history?"#f59e0b":"#06b6d4",fontSize:16,fontWeight:800,flexShrink:0}}>{index+1}</div>
      <div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,fontSize:17,color:"#e2e8f0"}}>{market.city}, {market.state}</div><div style={{fontSize:13,color:"#64748b",marginTop:1}}>{market.has_artist_history?`${market.historical?.prior_visits||0} visits`:"New"}</div></div>
      <div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:21,fontWeight:800,color:"#f59e0b",fontVariantNumeric:"tabular-nums"}}>{market.market_score.toFixed(1)}</div><div style={{fontSize:12,color:"#64748b",textTransform:"uppercase",letterSpacing:.5}}>score</div></div>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:10}}>
      {[{label:"Fill",value:pct(market.predicted_fill_probability),pct:market.predicted_fill_probability},{label:"Cap",value:fmtN(market.predicted_capacity),pct:Math.min(market.predicted_capacity/20000,1)},{label:"Income",value:fmt(p?.total_artist_income||market.predicted_net_revenue),pct:Math.min((p?.total_artist_income||market.predicted_net_revenue)/500000,1)}].map(s=>(
        <div key={s.label} style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:2,alignItems:"baseline"}}><span style={{fontSize:12,color:"#64748b",textTransform:"uppercase",letterSpacing:.3}}>{s.label}</span><span style={{fontSize:13,fontWeight:600,color:"#cbd5e1",fontVariantNumeric:"tabular-nums"}}>{s.value}</span></div><ScoreBar value={s.pct*100}/></div>
      ))}
    </div>
    {open&&(<div style={{marginTop:10,paddingTop:10,borderTop:"1px solid rgba(255,255,255,0.06)",fontSize:14,color:"#94a3b8",lineHeight:1.6}}>
      ...
      {market.has_artist_history&&market.historical&&(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"3px 12px",fontSize:13,color:"#64748b",marginTop:5}}><span>Prior fill: <b style={{color:"#cbd5e1"}}>{pct(market.historical.prior_avg_fill_rate||0)}</b></span><span>Last cap: <b style={{color:"#cbd5e1"}}>{fmtN(market.historical.prior_last_capacity||0)}</b></span></div>)}
    </div>)}
  </div>)}
...
function ProfitabilityView({data}){
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{background:"rgba(255,255,255,0.02)",borderRadius:10,border:"1px solid rgba(255,255,255,0.06)",padding:"14px 16px"}}>
      <div style={{fontSize:16,fontWeight:700,color:"#f59e0b",marginBottom:12}}>Tour P&L Summary</div>
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:14}}>
          <thead><tr style={{borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
            <th style={{textAlign:"left",padding:"6px 8px",color:"#64748b",fontWeight:600,fontSize:12,textTransform:"uppercase",letterSpacing:.5}}>Category</th>
            <th style={{textAlign:"right",padding:"6px 8px",color:"#64748b",fontWeight:600,fontSize:12,textTransform:"uppercase",letterSpacing:.5}}>Per Show Avg</th>
            <th style={{textAlign:"right",padding:"6px 8px",color:"#64748b",fontWeight:600,fontSize:12,textTransform:"uppercase",letterSpacing:.5}}>Tour Total</th>
          </tr></thead>
          <tbody>
      ...
      {fs.deal_type_breakdown&&<div style={{marginTop:10,display:"flex",gap:6,flexWrap:"wrap"}}>
        {Object.entries(fs.deal_type_breakdown).map(([type,count])=>(
          <span key={type} style={{padding:"3px 8px",borderRadius:12,fontSize:12,fontWeight:600,background:"rgba(139,92,246,0.1)",color:"#a78bfa",border:"1px solid rgba(139,92,246,0.2)"}}>{type} ({count})</span>
        ))}
      </div>}
    </div>
    <div style={{background:"rgba(255,255,255,0.02)",borderRadius:10,border:"1px solid rgba(255,255,255,0.06)",padding:"14px 16px"}}>
      <div style={{fontSize:16,fontWeight:700,color:"#f59e0b",marginBottom:12}}>Per-Market Income Breakdown</div>
      <div style={{display:"flex",flexDirection:"column",gap:6}}>
        {mkts.map((m,i)=>{const p=m.profitability;const net=p?.artist_net||m.predicted_net_revenue||0;const merch=p?.estimated_merch_revenue||0;const total=p?.total_artist_income||net;const guar=m.recommended_guarantee||0;return(
          <div key={m.market} style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:100,fontSize:13,color:"#94a3b8",flexShrink:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{m.city}</div>
            ...
            <div style={{width:60,fontSize:13,color:"#cbd5e1",textAlign:"right",fontFamily:"'JetBrains Mono',monospace",flexShrink:0}}>{fmt(total)}</div>
          </div>
        )})}
      </div>
...
function TimingView({data,artistKey}){
  ...
  if(!seasonal||seasonal.length===0)return(<div style={{padding:20,textAlign:"center",color:"#64748b",fontSize:16}}>No seasonality data available for this artist.</div>);
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:16}}>
    {isAggregate&&<div style={{background:"rgba(245,158,11,0.06)",borderRadius:8,border:"1px solid rgba(245,158,11,0.2)",padding:"8px 12px",fontSize:14,color:"#f59e0b"}}>Showing typical timing for <strong>{phaseLabel}</strong> artists — search this artist in your settlements to see their personal data.</div>}
    <div style={{background:"rgba(255,255,255,0.02)",borderRadius:10,border:"1px solid rgba(255,255,255,0.06)",padding:"14px 16px"}}>
      <div style={{fontSize:16,fontWeight:700,color:"#f59e0b",marginBottom:4}}>Monthly Fill Rate</div>
      <div style={{fontSize:13,color:"#64748b",marginBottom:12}}>{isAggregate?`Aggregate across ${phaseLabel} artists`:"Based on historical headline shows"}</div>
      <div style={{display:"flex",alignItems:"flex-end",gap:4,height:120}}>
        {seasonal.map((m,i)=>{const isBest=bestMonths.includes(MONTHS[m.m]);return(
          <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
            <div style={{fontSize:12,color:"#cbd5e1",fontFamily:"'JetBrains Mono',monospace"}}>{m.n>0?pct(m.f):""}</div>
            ...
            <div style={{fontSize:12,color:isBest?"#f59e0b":"#64748b",fontWeight:isBest?700:400}}>{MONTHS[m.m]}</div>
            <div style={{fontSize:10,color:"#475569"}}>{m.n>0?`${m.n}`:"-"}</div>
          </div>
        )})}
      </div>
    </div>
    <div style={{background:"rgba(255,255,255,0.02)",borderRadius:10,border:"1px solid rgba(255,255,255,0.06)",padding:"14px 16px"}}>
      <div style={{fontSize:16,fontWeight:700,color:"#f59e0b",marginBottom:4}}>Monthly Revenue</div>
      <div style={{fontSize:13,color:"#64748b",marginBottom:12}}>Average artist net by month</div>
      <div style={{display:"flex",alignItems:"flex-end",gap:4,height:120}}>
        {seasonal.map((m,i)=>{const isBest=bestMonths.includes(MONTHS[m.m]);return(
          <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
            <div style={{fontSize:12,color:"#cbd5e1",fontFamily:"'JetBrains Mono',monospace"}}>{m.n>0?fmt(m.r):""}</div>
            ...
            <div style={{fontSize:12,color:isBest?"#06b6d4":"#64748b",fontWeight:isBest?700:400}}>{MONTHS[m.m]}</div>
          </div>
        )})}
      </div>
    </div>
    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
      <div style={{padding:"6px 10px",borderRadius:6,fontSize:13,background:"rgba(245,158,11,0.08)",color:"#f59e0b",border:"1px solid rgba(245,158,11,0.2)"}}>Best: {bestMonths.join(", ")}</div>
      <div style={{padding:"6px 10px",borderRadius:6,fontSize:13,background:"rgba(100,116,139,0.08)",color:"#94a3b8",border:"1px solid rgba(100,116,139,0.2)"}}>Avoid: Jul, Aug (festival competition)</div>
    </div>
  </div>);
}
    ...
    <div style={{minHeight:"100vh",background:"#0a0a0f",color:"#e2e8f0",fontFamily:"'DM Sans','Helvetica Neue',sans-serif"}}>
      ...
      <div style={{padding:"16px",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
          <button onClick={()=>setSidebarOpen(!sidebarOpen)} style={{padding:"8px 12px",borderRadius:6,border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.05)",color:"#f59e0b",fontSize:26,cursor:"pointer",lineHeight:1}}>{"\u2630"}</button>
          <div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:700,letterSpacing:2,color:"#f59e0b",textTransform:"uppercase"}}>Wilder AI</div><div style={{fontSize:21,fontWeight:800,color:"#f8fafc",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>Tour Recommendation</div></div>
        </div>
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")handleSearch()}} placeholder="Search any artist..." style={{flex:1,padding:"10px 14px",borderRadius:8,border:"1px solid rgba(255,255,255,0.12)",background:"rgba(255,255,255,0.05)",color:"#f8fafc",fontSize:18,outline:"none",fontFamily:"inherit",boxSizing:"border-box"}}/>
          <button onClick={handleSearch} disabled={loading||!searchQuery.trim()} style={{padding:"10px 16px",borderRadius:8,border:"none",cursor:loading?"wait":"pointer",background:loading?"rgba(245,158,11,0.3)":"#f59e0b",color:"#0a0a0f",fontSize:17,fontWeight:700,fontFamily:"inherit",opacity:!searchQuery.trim()?0.4:1,whiteSpace:"nowrap"}}>
        ...
        <div style={{padding:"6px 10px",borderRadius:6,fontSize:13,fontWeight:600,background:"rgba(245,158,11,0.1)",color:"#f59e0b",border:"1px solid rgba(245,158,11,0.2)",display:"inline-block"}}>{data.model_metadata?.n_training_shows||__N_SHOWS__} shows &middot; {data.model_metadata?.n_markets||__N_MARKETS__} markets &middot; {data.model_metadata?.n_features||__N_FEATURES__} features</div>
      </div>

      <div style={{display:"flex",gap:0,minHeight:"calc(100vh - 140px)",position:"relative"}}>
        {/* Sidebar */}
        <div style={{width:sidebarOpen?240:0,borderRight:sidebarOpen?"1px solid rgba(255,255,255,0.06)":"none",padding:sidebarOpen?"16px 12px":0,flexShrink:0,transition:"all 0.3s",overflowY:sidebarOpen?"auto":"hidden",overflowX:"hidden",position:"fixed",left:0,top:0,bottom:0,background:"#0a0a0f",zIndex:200}}>
          <div style={{fontSize:12,fontWeight:700,letterSpacing:1.5,color:"#64748b",textTransform:"uppercase",marginBottom:8}}>Demo Artists</div>
          {DEMOS.map(s=>(
            <div key={s.key} onClick={()=>{setActiveDemo(s.key);setData(PRECOMPUTED[s.key]);setSearchQuery("");setError(null);setSidebarOpen(false)}} style={{padding:"8px 10px",borderRadius:6,cursor:"pointer",marginBottom:4,background:activeDemo===s.key?"rgba(245,158,11,0.08)":"transparent",border:`1px solid ${activeDemo===s.key?"rgba(245,158,11,0.2)":"transparent"}`,transition:"all 0.15s"}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:21}}>{s.icon}</span><div><div style={{fontSize:16,fontWeight:700,color:activeDemo===s.key?"#f59e0b":"#e2e8f0"}}>{s.label}</div><div style={{fontSize:12,color:"#64748b"}}>{s.sub}</div></div></div>
            </div>))}
          ...
          <div style={{marginTop:16,padding:"10px",background:"rgba(255,255,255,0.02)",borderRadius:6,border:"1px solid rgba(255,255,255,0.05)"}}>
            <div style={{fontSize:12,fontWeight:700,letterSpacing:1.5,color:"#64748b",textTransform:"uppercase",marginBottom:6}}>Artist Profile</div>
            <div style={{fontSize:18,fontWeight:800,color:"#f8fafc",marginBottom:4}}>{data.artist}</div>
            <div style={{display:"inline-block",padding:"2px 8px",borderRadius:12,fontSize:13,fontWeight:700,background:`${PC[phase]||"#6b7280"}20`,color:PC[phase]||"#6b7280",border:`1px solid ${PC[phase]||"#6b7280"}40`,marginBottom:6}}>{PL[phase]||phase}</div>
            ...
            {data.ai_summary&&(<div style={{marginTop:8,padding:"6px 8px",background:"rgba(6,182,212,0.06)",borderRadius:4,border:"1px solid rgba(6,182,212,0.15)",fontSize:13,color:"#94a3b8",lineHeight:1.5}}><div style={{fontSize:12,fontWeight:700,color:"#06b6d4",marginBottom:3}}>AI ANALYSIS</div>{data.ai_summary}</div>)}
          </div>
        </div>

        {/* Backdrop */}
        {sidebarOpen&&<div onClick={()=>setSidebarOpen(false)} style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",zIndex:199}}/>}
        ...
        <div style={{flex:1,padding:"16px",overflow:"auto",width:"100%"}}>
          ...
          <div style={{display:"flex",gap:4,marginBottom:12,overflowX:"auto"}}>
            {[{k:"map",l:"Map"},{k:"markets",l:"Markets"},{k:"profitability",l:"Profitability"},{k:"route",l:"Route"},{k:"timing",l:"Timing"}].map(t=>(<button key={t.k} onClick={()=>setView(t.k)} style={{padding:"8px 14px",borderRadius:6,border:"1px solid",borderColor:view===t.k?"rgba(245,158,11,0.3)":"rgba(255,255,255,0.08)",background:view===t.k?"rgba(245,158,11,0.08)":"transparent",color:view===t.k?"#f59e0b":"#94a3b8",fontSize:14,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>{t.l}</button>))}
          </div>

          {view==="map"&&(<div style={{background:"rgba(255,255,255,0.02)",borderRadius:10,border:"1px solid rgba(255,255,255,0.06)",overflow:"hidden"}}><MapViz data={data}/><div style={{padding:"10px 14px",borderTop:"1px solid rgba(255,255,255,0.06)",display:"flex",gap:12,flexWrap:"wrap",fontSize:13,color:"#64748b"}}><span><span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",background:"#f59e0b",marginRight:4}}/>Known</span><span><span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",background:"#06b6d4",marginRight:4}}/>New</span></div></div>)}
          ...
          {view==="route"&&(<div style={{background:"rgba(255,255,255,0.02)",borderRadius:10,border:"1px solid rgba(255,255,255,0.06)",padding:"14px 16px"}}><div style={{fontSize:16,fontWeight:700,color:"#f59e0b",marginBottom:12}}>Optimized Route &middot; {data.route.total_distance_miles.toLocaleString()} mi</div>
            {data.route.routed_markets.map((mkt,i)=>{const m=data.markets.find(x=>x.market===mkt);return(<div key={mkt} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<data.route.routed_markets.length-1?"1px solid rgba(255,255,255,0.04)":"none"}}><div style={{width:24,height:24,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(245,158,11,0.1)",color:"#f59e0b",fontSize:14,fontWeight:800,flexShrink:0}}>{i+1}</div><div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,color:"#e2e8f0",fontSize:17}}>{mkt}</div>{m&&<div style={{color:"#64748b",fontSize:13,marginTop:1}}>Score {m.market_score} &middot; {fmtN(m.predicted_capacity)} &middot; {fmt(m.profitability?.total_artist_income||m.predicted_net_revenue)} income</div>}</div>{i<data.route.routed_markets.length-1&&<div style={{color:"#334155",fontSize:21,flexShrink:0}}>{"\u2192"}</div>}</div>)})}</div>)}

          {view==="timing"&&<TimingView data={data} artistKey={activeDemo}/>}

          <div style={{marginTop:16,padding:"12px 14px",background:"rgba(255,255,255,0.02)",borderRadius:10,border:"1px solid rgba(255,255,255,0.06)"}}><div style={{fontSize:12,fontWeight:700,letterSpacing:1.5,color:"#64748b",textTransform:"uppercase",marginBottom:10}}>Model Features</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{Object.entries(data.model_metadata?.top_features||{}).map(([k,v])=>(<div key={k} style={{padding:"4px 8px",borderRadius:5,fontSize:13,fontFamily:"'JetBrains Mono',monospace",background:`rgba(245,158,11,${.05+v*.5})`,color:"#f59e0b",border:"1px solid rgba(245,158,11,0.15)"}}>{k.replace(/_/g," ")} <b>{(v*100).toFixed(0)}%</b></div>))}</div></div>
        </div>
      </div>
    </div>);
}
