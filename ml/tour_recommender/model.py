...
class TourRecommendationModel:
    ...
    def __init__(self):
        ...
        self.revenue_model = None     # E[artist_net] regressor
    ...
    def train(self, feature_store: FeatureStore):
        """Train all three prediction heads on the full training matrix."""
        tm = feature_store.training_matrix.copy()
        tm = tm.dropna(subset=ALL_FEATURES + ['target_fill_rate', 'target_artist_net', 'target_capacity'])

        X = tm[ALL_FEATURES].fillna(0).values
        self.scaler.fit(X)
        ...
        # ── Revenue Model (regression: predict artist net) ──
        y_rev = tm['target_artist_net'].values
        self.revenue_model = GradientBoostingRegressor(
            n_estimators=150, max_depth=4, learning_rate=0.1,
            subsample=0.8, min_samples_leaf=5, random_state=42)
        self.revenue_model.fit(X_scaled, y_rev)

        # ── Capacity Model (regression: predict appropriate capacity) ──
        ...
        self._evaluate(tm, X_scaled, y_fill, y_rev, y_cap)

        # Feature importances (averaged across models)
        for feat, imp_f, imp_r, imp_c in zip(
            ALL_FEATURES,
            self.fill_model.feature_importances_,
            self.revenue_model.feature_importances_,
            self.capacity_model.feature_importances_
        ):
            self.feature_importances[feat] = round(float((imp_f + imp_r + imp_c) / 3), 4)
        ...
        print(f"  Revenue R²: {self.metrics.revenue_r2}, MAE: ${self.metrics.revenue_mae:,.0f}")
        print(f"  Capacity R²: {self.metrics.capacity_r2}")

        return self

    def _evaluate(self, tm, X_scaled, y_fill, y_rev, y_cap):
        ...
        if len(unique_artists) < 2:
            # Can't do artist-level CV with < 2 artists; use simple scoring
            ...
            self.metrics.revenue_r2 = round(r2_score(y_rev, self.revenue_model.predict(X_scaled)), 3)
            self.metrics.revenue_mae = round(mean_absolute_error(y_rev, self.revenue_model.predict(X_scaled)), 0)
        ...
        fill_aucs, rev_r2s, rev_maes, cap_r2s = [], [], [], []
        ...
        for train_idx, test_idx in gkf.split(X_scaled, groups=artists):
            ...
            fm = GradientBoostingClassifier(n_estimators=100, max_depth=3, random_state=42)
            fm.fit(X_tr, y_f_tr)
            ...
            rm = GradientBoostingRegressor(n_estimators=100, max_depth=3, random_state=42)
            rm.fit(X_tr, y_r_tr)
            preds = rm.predict(X_te)
            rev_r2s.append(r2_score(y_r_te, preds))
            rev_maes.append(mean_absolute_error(y_r_te, preds))

            # Capacity
            y_c_tr, y_c_te = y_cap[train_idx], y_cap[test_idx]
            cm = GradientBoostingRegressor(n_estimators=100, max_depth=3, random_state=42)
            cm.fit(X_tr, y_c_tr)
    ...
    def predict_for_markets(
        ...
        """
        Predict fill probability, revenue, and capacity for an artist across markets.
        Works for known artists (from settlement data) AND unknown artists
        (profile built from Chartmetric/Spotify).
        """
        ...
        for mkt in markets:
            ...
            # Artist × Market history (zero if unknown artist or new market)
            am = am_history[
            ...
            pred_revenue = max(0, self.revenue_model.predict(X_scaled)[0])
